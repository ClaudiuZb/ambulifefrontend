...
const CNASBookings = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const response = await axios.get('/api/cnas-services');
      
      console.log('Răspuns API programări CNAS:', response.data);
      ...
      if (response.data && response.data.data && Array.isArray(response.data.data)) {
        ...
        console.log('Date programări CNAS procesate:', bookingsData);
  ...
  const handleUploadDocument = async (bookingId) => {
    ...
    try {
      const response = await axios.post(
        `/api/cnas-services/${bookingId}/documents`,
        formData,
        {
          headers: {
      ...
      if (response.data && response.data.success && response.data.data) {
        ...
        try {
          const bookingResponse = await axios.get(`/api/cnas-services/${bookingId}`);
          
          if (bookingResponse.data && bookingResponse.data.success) {
            const updatedBooking = bookingResponse.data.data;
            
            // Formatăm data și timpul pentru afișare
            const serviceDate = new Date(updatedBooking.date);
            const bookingDate = serviceDate.toISOString().split('T')[0];
            const bookingTime = serviceDate.toTimeString().slice(0, 5);
  ...
  const handleDeleteDocumentConfirm = async () => {
    try {
      ...
      await axios.delete(`/api/cnas-services/${bookingId}/documents/${documentId}`);
  ...
  const handleDownloadDocument = async (bookingId, documentId, documentName) => {
    try {
      ...
      const response = await axios.get(`/api/cnas-services/${bookingId}/documents/${documentId}/download`, {
        responseType: 'blob'
      });
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`/api/cnas-services/${bookingToDelete._id}`);
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      ...
      const response = await axios.put(`/api/cnas-services/${bookingToUpdateStatus._id}`, updateData);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Trimit date pentru actualizarea programării:', cnasServiceData);
      
      const response = await axios.put(`/api/cnas-services/${bookingToEdit._id}`, cnasServiceData);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
          ...
          console.log('Trimit date privind programarea CNAS:', cnasServiceData);
          
          const response = await axios.post('/api/cnas-services', cnasServiceData);
          ...
          if (selectedFile) {
            try {
              ...
              const uploadResponse = await axios.post(
                `/api/cnas-services/${createdService._id}/documents`,
                docFormData,
                {
                  headers: {
                    'Content-Type': 'multipart/form-data'
    ...
    const renderBookingsTable = () => (
      <div className="content-card">
        <h5 className="mb-4">
          ...
          Programări CNAS ({filteredBookings.length})
        </h5>
        ...
        {filteredBookings.length === 0 && !loading && (
          <div className="text-center py-4">
            <p className="text-muted">Nu există programări CNAS înregistrate.</p>
            <button 
              className="btn btn-success mt-3"
              onClick={handleAddNewBooking}
            >
              <FontAwesomeIcon icon={faPlus} className="me-2" />
              Adaugă prima programare CNAS
            </button>
          </div>
        )}
      </div>
    ...
    const renderViewBookingModal = () => (
        <Modal show={showViewModal} onHide={() => setShowViewModal(false)} size="lg" centered className="booking-modal">
          <Modal.Header closeButton>
            <Modal.Title>Detalii programare CNAS</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {currentBooking && (
